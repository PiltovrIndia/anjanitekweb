...
const getAllPaymentsDataAPI = async (pass, offset, role) => 
...
export default function Payments() {
    ...
    useEffect(()=>{
        let cookieValue = biscuits.get('sc_user_detail')
            if(cookieValue){
                ...
                // if(!completed){
                //     getAllPayments(initialDatesValues.from,initialDatesValues.to);
                // }
                // else {
    ...
    async function getAllPayments(){
        ...
        try {    
            const result  = await getAllPaymentsDataAPI(process.env.NEXT_PUBLIC_API_PASS, offset, JSON.parse(decodeURIComponent(biscuits.get('sc_user_detail'))).role) 
