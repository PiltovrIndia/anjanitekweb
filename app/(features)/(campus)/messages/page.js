...
export default function Messages() {
    ...
    async function getData(){
        ...
        try {    
            const result  = await getSenders(process.env.NEXT_PUBLIC_API_PASS, JSON.parse(decodeURIComponent(biscuits.get('sc_user_detail'))).userId)
        ...
        <div className={styles.horizontalsection} style={{height:'100%', width:'100%'}}>
                ...
                <div className='flex flex-row gap-2' key={1234} style={{height:'100%', width:'100%', }}>
                    ...
                    <ScrollArea className="w-72 rounded-md border">
                        ...
                        <div className="w-[580px] flex flex-col rounded-md border p-4 gap-4">
                                ...
                                {/* <div className="grid w-full items-center gap-4"> */}
                                    {searchingMessages ? <Skeleton className="h-4 w-[300px] h-[100px]" /> :
                                        ...
                                        senderMessagesList.map((message, index) => (
                                            <div key={index} className="w-fit flex flex-col rounded-md border p-2" style={(message.sender==JSON.parse(decodeURIComponent(biscuits.get('sc_user_detail'))).userId) ? {alignSelf:'self-end'} : {alignSelf:'self-start'}} ref={index === senderMessagesList.length - 1 ? lastItemRef : null}>
                                                <Label className="p-1">{message.message}</Label>
                                                {/* <Label className="text-gray-500 p-1">{message.sender}</Label> */}
                                                <p className="text-xs text-gray-500 p-1">{dayjs(message.sentAt).format('MMMM D, YYYY DD:mm A')}</p>
                                                
                                                {(message.sender==JSON.parse(decodeURIComponent(biscuits.get('sc_user_detail'))).userId) ? 
                                                (message.seen == 1) ? 
