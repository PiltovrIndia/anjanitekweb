...
export async function GET(request,{params}) {
    ...
    try{
        ...
        if(await Keyverify(params.ids[0])){
            ...
            // check for the users role
            // if SuperAdmin, get all the requests w.r.t status
            else if(params.ids[1] == 'GlobalAdmin' || params.ids[1] == 'SuperAdmin' || params.ids[1] == 'OutingAdmin'){
                
                // First Query: Get visitorrequests and matching users information
                const [rows, fields] = await connection.execute('SELECT p.*, u.* FROM visitorpass p JOIN users u ON p.collegeId = u.collegeId WHERE p.vStatus = ? and u.campusId=? ORDER BY p.requestDate DESC LIMIT 20 OFFSET ?', [params.ids[2], params.ids[5], params.ids[3]]);
            ...
            // if OutingAssistant, get all requests that are issued by OutingAdmin or SuperAdmin
                ...
                // First Query: Get visitorrequests and matching users information
                const [rows, fields] = await connection.execute('SELECT p.*, u.* FROM visitorpass p JOIN users u ON p.collegeId = u.collegeId WHERE p.isOpen = 1 AND p.collegeId = ? ORDER BY p.requestDate DESC LIMIT 20 OFFSET ?', [params.ids[2], params.ids[3]]);
