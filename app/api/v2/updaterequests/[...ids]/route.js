        ...
        if(await Keyverify(params.ids[0])){
                ...
                if(params.ids[10] == 'Single'){
                    ...
                    try {
                        ...
                        if(params.ids[6] == 'Approved'){
                            ...
                            // check if the request is temporary pass and reject the pass requests
                            const [rows1, fields1] = await connection.execute(`UPDATE passrequest SET requestStatus="Rejected" where requestId LIKE "`+params.ids[2]+`"`);
                            connection.release();
                        ...
                        if(updatedRequestStatus == 'Issued'){
                            ...
                            if(rows[0].checkoutOn == null){
                                ...
                                if(dayjs(params.ids[2]).diff(dayjs(rows[0].requestFrom), 'minute') >= -60 && dayjs(params.ids[2]).isBefore(dayjs(rows[0].requestTo))){
                                    ...
                                    // student is not allowed to checkout way before. Show warning.
                                    return Response.json({status: 198, message:'Not allowed. Your checkout time is at '+ dayjs(rows[0].requestFrom).format('hh:mm A, DD-MM-YY')}, {status: 200})
                                }
                            ...
                            if(dayjs(params.ids[2]).diff(dayjs(rows[0].checkoutOn), 'minute') > 5){
                                ...
                                if(rows[0].requestType == '4'){
                                    ...
                                    // check if its the last day of day pass or the day later than last day
                                    // if so update the main request status to returned too
                                    if(dayjs(params.ids[2]).isSame(dayjs(rows[0].requestTo), 'day') || dayjs(params.ids[2]).isAfter(dayjs(rows[0].requestTo), 'day') ){
                                        // mark as returned
                                        ...
                                        var qq = `UPDATE passrequest SET requestStatus ="Returned", returnedOn = "`+params.ids[2]+`" WHERE requestId = "`+rows[0].requestId+`" AND returnedOn IS NULL`;
                                        const [passRows, passFields] = await connection.execute(qq);
            ...
            // this is when student or admin cancels the request.
                ...
                try {
                    ...
                    const [rows, fields] = await connection.execute('UPDATE request SET isOpen = 0, requestStatus="Cancelled", comment = CASE WHEN comment = "-" THEN REPLACE(comment,"-","'+remark+'") ELSE CONCAT(comment,"'+remark+'") END where requestId = "'+params.ids[2]+'" and isOpen=1');
                    connection.release();

                    // check if the request is temporary pass and cancel the pass requests
                    const [rows1, fields1] = await connection.execute('UPDATE passrequest SET requestStatus="Cancelled" where requestId = "'+params.ids[2]);
                    connection.release();
