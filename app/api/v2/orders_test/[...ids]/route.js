...
import { recordOrderAction, resolveOrderActionActor } from '../../orders/order-action-audit';
        ...
        if(await Keyverify(params.ids[0])){
            ...
            // Reject by admin
                ...
                try {
                    ...
                    await recordOrderAction(rejectConnection, {
                        orderId,
            ...
            // Mark as Out of Stock by Admin
                ...
                try {
                    ...
                    await recordOrderAction(oosConnection, {
                        orderId,
                        cartId: existingOrder.cartId,
                        actor,
                        actionType: 'OutOfStock',
                        actionOn: actionDate,
                        actionNotes: notes,
                    });
            ...
            // soft delete
                ...
                try {
                    ...
                    const [orderRows] = await connection.query(
                        `SELECT id, cartId FROM orders WHERE id = ? AND isDeleted = 0 FOR UPDATE`,
                        [orderId]
                    );
                    ...
                    const [result] = await connection.query(
                        `UPDATE orders
                         SET status = 'Deleted', isDeleted = 1, modifiedOn = ?
                         WHERE id = ?
                         AND isDeleted = 0
            ...
            // /U0.5/$cartId/$adminId/$actionDate
                ...
                try {
                    ...
                    for (const item of items.filter((item) => !['Cancelled', 'Rejected'].includes(item.status))) {
                        await recordOrderAction(saleOrderConnection, {
                            orderId: item.id,
                            cartId: item.cartId,
                            actor,
            ...
            // /U0.7/$orderId?actorId=$actorId
                ...
                try {
                    ...
                    if (result.affectedRows > 0) {
                        await recordOrderAction(connection, {
                            orderId,
                            cartId: ordersForReview[0]?.cartId,
                            actor,
                            actionType: 'InReview',
                            actionOn: new Date(),
                        });
