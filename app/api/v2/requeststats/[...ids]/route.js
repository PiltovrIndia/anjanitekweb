...
export async function GET(request,{params}) {
    ...
    try{
        ...
        if(await Keyverify(params.ids[0])){
            ...
            if(params.ids[1] == 'GlobalAdmin' || params.ids[1] == 'SuperAdmin' || params.ids[1] == 'Admin' || params.ids[1] == 'OutingAdmin' || params.ids[1] == 'OutingIssuer' ){

                var q = '';
                    
                // check if the level of stats are basic or detailed
                if(params.ids[4] == 1){ // basic
                    ...
                    if(params.ids[3] == 'All' && params.ids[1] != 'Admin'){
                        ...
                        if(params.ids[5]!=null){ // check if campus parameter is available
                            if(params.ids[5] == 'All'){
                                // q = 'SELECT s.status AS requestStatus, COUNT(r.requestStatus) AS count FROM (SELECT "Submitted" AS status UNION SELECT "Approved" UNION SELECT "Issued" UNION SELECT "InOuting" UNION SELECT "Rejected" UNION SELECT "Cancelled" UNION SELECT "Returned") AS s LEFT JOIN request r ON s.status = r.requestStatus AND r.isOpen = 1 GROUP BY s.status UNION SELECT "InCampus" AS requestStatus, COUNT(*) AS COUNT FROM users WHERE type = "hostel"';
                                q = 'SELECT s.status AS requestStatus, COALESCE(CAST(SUM(r.requestStatus = s.status) AS SIGNED), 0) AS count FROM (SELECT "Submitted" AS status UNION ALL SELECT "Approved" UNION ALL SELECT "Issued" UNION ALL SELECT "InOuting" UNION ALL SELECT "Rejected" UNION ALL SELECT "Cancelled" UNION ALL SELECT "Returned") AS s LEFT JOIN (SELECT * FROM request) AS r ON s.status = r.requestStatus GROUP BY s.status UNION ALL SELECT "InCampus" AS requestStatus, COUNT(*) AS count FROM users WHERE role = "student" AND type = "hostel"';
                            }
                            else {
                                q = 'SELECT s.status AS requestStatus, COALESCE(CAST(SUM(r.requestStatus = s.status) AS SIGNED), 0) AS count FROM (SELECT "Submitted" AS status UNION ALL SELECT "Approved" UNION ALL SELECT "Issued" UNION ALL SELECT "InOuting" UNION ALL SELECT "Rejected" UNION ALL SELECT "Cancelled" UNION ALL SELECT "Returned") AS s LEFT JOIN (SELECT * FROM request WHERE campusId = "'+params.ids[5]+'") AS r ON s.status = r.requestStatus GROUP BY s.status UNION ALL SELECT "InCampus" AS requestStatus, COUNT(*) AS count FROM users WHERE role = "student" AND type = "hostel" AND campusId = "'+params.ids[5]+'"';
                                
                                //select requestStatus as requestStatus, count(*) as count from request where requestStatus IN ("Submitted","Approved","Issued","InOuting","Rejected","Cancelled","Returned") AND campusId="'+params.ids[5]+'" GROUP BY requestStatus UNION select "InCampus" as requestStatus, count(*) as count from users where role="student" and type="hostel" and campusId="'+params.ids[5]+'"';
                            
                                // q = 'SELECT s.status AS requestStatus, COUNT(r.requestStatus) AS count FROM (SELECT "Submitted" AS status UNION SELECT "Approved" UNION SELECT "Issued" UNION SELECT "InOuting" UNION SELECT "Rejected" UNION SELECT "Cancelled" UNION SELECT "Returned") AS s LEFT JOIN request r ON s.status = r.requestStatus and r.campusId = "'+params.ids[5]+'" AND r.isOpen = 1 GROUP BY s.status UNION SELECT "InCampus" AS requestStatus, COUNT(*) AS COUNT FROM users WHERE type = "hostel" and campusId="?"';
                            }
                        }
                        else {
                            q = 'SELECT s.status AS requestStatus, COUNT(r.requestStatus) AS count FROM (SELECT "Submitted" AS status UNION SELECT "Approved" UNION SELECT "Issued" UNION SELECT "InOuting" UNION SELECT "Rejected" UNION SELECT "Cancelled" UNION SELECT "Returned") AS s LEFT JOIN request r ON s.status = r.requestStatus AND r.isOpen = 1 GROUP BY s.status UNION SELECT "InCampus" AS requestStatus, COUNT(*) AS COUNT FROM users WHERE type = "hostel"';
                        ...
                        if(params.ids[5]!=null && params.ids[5] != 'All'){
                            conditionsString = conditionsString + " AND campusId='"+params.ids[5]+"' "
                        }
