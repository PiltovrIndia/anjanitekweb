    ...
    try{
        ...
        if(await Keyverify(params.ids[0])){
            ...
            // if(params.ids[2] != 3){
              ...
              var universityId = '-';
              if(params.ids[15]!=null){
                universityId = params.ids[15];
              }

              // these are useful when raising emergency outing by admin
              var adminId = '-',adminName = '-',approvedOn=null,issuedOn=null,status = 'Submitted';
              if(params.ids[16]!=null){
                status = 'Issued';
                adminId = params.ids[16];
                adminName = params.ids[17];
                approvedOn = params.ids[10];
                issuedOn = params.ids[10];
              }

                try {

                  // console.log(params.ids[6]);
                  // console.log(dayjs(params.ids[6])); // request From
                  // console.log(dayjs(params.ids[6]).format('YYYY-MM-DD HH:mm:ss'));
                  
                  // console.log(dayjs(params.ids[10])); // request Date
                  // console.log(dayjs(params.ids[10]).format('YYYY-MM-DD HH:mm:ss'));
                  
                  // console.log(dayjs(new Date(Date.now())).format('YYYY-MM-DD HH:mm:ss'));
                  ...
                  const [results, fields1] = await connection.execute('SELECT NOW()',[]);
                    ...
                    // 'results' is an array with one element for each row of results
                    // 'NOW()' returns the current date and time, so we expect one row with one column
                    // console.log('The current date and time in MySQL is: ', results[0]['NOW()']);
                    // console.log('The current date and time in MySQL is: ', dayjs(results[0]['NOW()']));
                  ...
                  // if(dayjs(new Date(Date.now())).isBefore( dayjs(params.ids[6]))){
                    ...
                    if(rows0.length == 0){
                      ...
                      // check if the users is not blocked by the admin
                      // if profileUpdated column value is 3, then users is meant to be blocked by admin.
                      const [rows1, fields1] = await connection.execute('SELECT branch, profileUpdated from users where collegeId=?', [ params.ids[4] ]);
                      
                      if(rows1[0].profileUpdated == 3){
                        // mention that admin blocked the users to raise request
                        return Response.json({status: 199, message:'You are not allowed to raise request. Contact your admin!'}, {status: 201})
                      }
                      else {

                        // create query for insert
                        const q = 'INSERT INTO request (requestId, universityId, campusId, requestType, oRequestId, collegeId, description, requestFrom, requestTo, duration, requestStatus, requestDate, approver, approverName, approvedOn, comment, issuer, issuerName, issuedOn, consentBy, isOpen, isStudentOut, checkoutOn, returnedOn, isAllowed) VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ? )';
                        // create new request
                        const [rows, fields] = await connection.execute(q, [ params.ids[1], universityId, campusId, params.ids[2], params.ids[3], params.ids[4], decodeURIComponent(params.ids[5]), params.ids[6], params.ids[7], params.ids[8], status, params.ids[10] ,  adminId,adminName, approvedOn, '-', adminId,adminName,issuedOn, consent, 1, 0, null, null, params.ids[9]]);
                        ...
                        const [nrows, nfields] = await connection.execute('SELECT gcm_regId FROM `users` where role IN ("SuperAdmin") or (role="Admin" AND branch LIKE "%'+rows1[0].department+'-'+rows1[0].branch+'-'+rows1[0].year+'%")');
                        ...
                        // return Response.json({status: 200, message:'Request submitted!'+dayjs(params.ids[6]).format('YYYY-MM-DD HH:mm:ss')+"Server date:"+dayjs(new Date(Date.now())).format('YYYY-MM-DD HH:mm:ss')+"SQL time:"+sqlTime  , notification: notificationResult}, {status: 200})
                    ...
                    // return Response.json({status: 199, message:'Your checkout date and time is in past. Update to future time!'+dayjs(params.ids[6]).format('YYYY-MM-DD HH:mm:ss')+"Server date:"+dayjs(new Date(Date.now())).format('YYYY-MM-DD HH:mm:ss')+"SQL time:"+sqlTime}, {status: 201})
